...
export class Mpris extends Emitter<MprisEvents> {
    ...
    protected constructor(
        protected connection: DBus.MessageBus,
        protected dbus: DBusInterface
    ) {
        super();
    ...
    protected async initPlayers() {
        const allNames = await this.dbus.ListNames();
        const playerNames = allNames
            .filter((name) => name.startsWith(MPRIS_SERVICE_PREFIX))
            .map((name) => name.substring(MPRIS_SERVICE_PREFIX.length));

        playerNames.forEach((id) => this.createPlayer(id));

        this.dbus.on("NameOwnerChanged", (name, ownerOld, ownerNew) => {
            console.log("NameOwnerChanged", { name, ownerOld, ownerNew });

            if (!name.startsWith(MPRIS_SERVICE_PREFIX)) return;

            if (ownerOld !== "") {
                // only destroy the old one if there was one
                this.destroyPlayer(name.substring(MPRIS_SERVICE_PREFIX.length));
            }
            if (ownerNew !== "") {
                //only create the new one if there is one
    ...
    public static async create() {
        ...
        const dbusObject = await connnection.getProxyObject(
            DBUS_SERVICE,
            DBUS_PATH
        );
        const dbusInterface =
            dbusObject.getInterface<DBusInterface>(DBUS_INTERFACE);
