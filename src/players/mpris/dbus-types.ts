...
export interface DBusInterface extends DBus.ClientInterface {
    ...
    on(
        e: "NameOwnerChanged",
        listener: (name: string, old_owner: string, new_owner: string) => void
    ): this;
}
...
export interface TypedPropertiesInterface<Path extends InterfaceMap>
    extends DBus.ClientInterface {
    Get<IF extends InterfaceOf<Path>, P extends PropOf<Path[IF]>>(
        interfaceName: IF,
        propName: P
    ): Promise<DBus.Variant<Path[IF][P]>>;
    Set<IF extends InterfaceOf<Path>, P extends PropOf<Path[IF]>>(
        interfaceName: IF,
        propName: P,
        value: DBus.Variant<Path[IF][P]>
    ): Promise<void>;
    GetAll<IF extends InterfaceOf<Path>>(
        interfaceName: IF
    ): Promise<Variantify<Path[IF]>>;

    on<IF extends InterfaceOf<Path>>(
        event: "PropertiesChanged",
        listener: (
            interfaceName: IF,
            changedProps: Partial<Variantify<Path[IF]>>,
            invalidatedProps: PropOf<Path[IF]>[]
        ) => void
    ): this;
}
...
export interface MPRISPlayerIntrefaceProps
    extends Omit<PlayerInterfaceProps, "Position"> {
    Position: BigInt;
}
